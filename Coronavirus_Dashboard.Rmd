...
pal <- colorFactor(c("#E95555"), domain = c("California", "New York", "Connecticut", "New Jersey", "Delaware", "Illinois", "Florida", "Nevada", "Louisiana", "Ohio"), na.color = "#81C77F")
...
lock_states <- list("US-CA", "US-IL", "US-CT", "US-NY", "US-NJ", "US-DE", "US-OH", "US-LA", "US-NV", "US-FL")
lock_names <- list("ca_time", "il_time", "ct_time", "ny_time", "nj_time", "de_time", "oh_time", "la_time", "nv_time", "fl_time")

## Not locked down states

not_lock_states <- list("US-AL", "US-AK", "US-AZ", "US-AR", "US-CO", "US-GA", "US-HI", "US-ID", "US-IN", "US-IA", "US-KS", "US-KY", "US-ME", "US-MD", "US-MA", "US-MI", "US-MN", "US-MS", "US-MO", "US-MT", "US-NE", "US-NH", "US-NM", "US-NC", "US-ND", "US-OK", "US-OR", "US-PA", "US-RI", "US-SC", "US-SD", "US-TN", "US-TX", "US-UT", "US-VT", "US-VA", "US-WA", "US-WV", "US-WI", "US-WY")
not_lock_names <- list("al_time", "ak_time", "az_time", "ar_time", "co_time", "ga_time", "hi_time", "id_time", "ind_time", "ia_time", "ks_time", "ky_time", "me_time", "md_time", "ma_time", "mi_time", "mn_time", "ms_time", "mo_time", "mt_time", "ne_time", "nh_time", "nm_time", "nc_time", "nd_time", "ok_time", "or_time", "pa_time", "ri_time", "sc_time", "sd_time", "tn_time", "tx_time", "ut_time", "vt_time", "va_time", "wa_time", "wv_time", "wi_time", "wy_time")
...
for (i in lock_states) {
  ls <- gtrends(keyword="Depression", geo=i, time="today 1-m")
...
for (p in not_lock_states) {
  nls <- gtrends(keyword="Depression", geo=p, time="today 1-m")
...
l_list <- list()
for (i in lock_states) {
  ls <- gtrends(keyword="Domestic violence", geo=i, time="today 1-m")
  ls <- data.frame(ls$interest_over_time["date"], ls$interest_over_time["hits"])
  for (n in lock_names) {
    n <- ls
    l_list[[i]] <- n
...
n_list <- list()
for (p in not_lock_states) {
  nls <- gtrends(keyword="Domestic violence", geo=p, time="today 1-m")
  nls <- data.frame(nls$interest_over_time["date"], nls$interest_over_time["hits"])
  for (k in not_lock_names) {
    k <- nls
    n_list[[p]] <- k
...
l_list <- list()
for (i in lock_states) {
  ls <- gtrends(keyword="Suicide", geo=i, time="today 1-m")
  ls <- data.frame(ls$interest_over_time["date"], ls$interest_over_time["hits"])
  for (n in lock_names) {
    n <- ls
    l_list[[i]] <- n
...
n_list <- list()
for (p in not_lock_states) {
  nls <- gtrends(keyword="Suicide", geo=p, time="today 1-m")
  nls <- data.frame(nls$interest_over_time["date"], nls$interest_over_time["hits"])
  for (k in not_lock_names) {
    k <- nls
    n_list[[p]] <- k
...
More U.S. states are entering lockdown. Under lockdown, people can't go outside except for "essential" businesses. Such businesses often include pharmacy, hospital, and grocery that provide sustenance. They exclude dine-in restaurants, parks, and gyms where people usually gather to socialize. The graphs show how often people in each type of states -- locked down or not locked down -- search terms that represent change in their mental health and living environment. The map gives context by showing relative number of total coronavirus cases by state and by the two types. 
...
For any inquiries/feedback, please email David: slee511@jhu.edu. Especially if I'm missing a locked-down state. 
