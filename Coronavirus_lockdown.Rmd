---
title: "Coronavirus Cases & Deaths in U.S. and the World"
output: 
...
confirm <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/03-25-2020.csv")
...
leaflet(world_deaths) %>% addTiles() %>%
  addCircleMarkers(lng = ~Long, lat = ~Lat, weight = 1,
    radius = ~sqrt(world_deaths[, length(world_deaths)])/7, popup = paste("Country: ", world_deaths$Country.Region, "<br>", "Province: ", world_deaths$Province.State, "<br>", "Cases: ", world_deaths[, length(world_deaths)]), stroke=F, color="black" ,fillOpacity = 0.7)
